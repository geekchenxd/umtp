...
#if 0
int umtp_encrypt(uint8_t *input, uint32_t input_len, uint8_t *output)
{
...
int umtp_decrypt(uint8_t *input, uint32_t input_len, uint8_t *output)
{
	OpenSSL_add_all_algorithms();
	ERR_load_crypto_strings();     
...
int main(int arc, char *argv[])
{
	...
	len = umtp_encrypt(raw, 12, encrypt_raw);
	if (len > 0) {
	...
	len = umtp_decrypt(encrypt_raw, len, decrypt);
	if (len > 0) {
