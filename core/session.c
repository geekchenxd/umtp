...
static struct umtp_session session[MAX_FTTP_SESSION];
static void (*umtp_session_failed_handle)(uint8_t);
	...
	for (i = 0; i < MAX_FTTP_SESSION; i++) {
		...
		if (session[i].timer == 0) {
			if (session[i].retry < apdu_retry()) {
				...
				session[i].retry++;
				/*send pdu*/
				umtp_send_udp(&session[i].dest, 
						&session[i].pdu[0],session[i].pdu_len);
			} else {
				/*handle failed and clear session id*/
				if (umtp_session_failed_handle) {
					umtp_session_failed_handle(session[i].session_id);
				}
...
{
	if (fun)
		umtp_session_failed_handle = fun;
}
